...
saw_func x = 2 * (x - fromIntegral (round (x - 0.5))) - 1
...
tri_func :: (Fractional a, RealFrac a) => a -> a
tri_func x = 4 * abs ((x - 0.25) - fromIntegral (round (x - 0.75)) - 0.5) - 1

tri_wave :: Float -> Float -> Samples -> (Samples,[Float])
tri_wave pitch duration fs = (fs * duration , map tri_func $ map (*step) [0.0, 1/fs .. duration-1/fs])
                              where
                                step = pitch
...
gen_wave wave_form (pitch:duration:_) |wave_form == "sin" = let (x,xs) = sin_wave pitch duration sample_rate in (x:xs)
                                       ...
                                       | wave_form == "tri" = let (x,xs) = tri_wave pitch duration sample_rate in (x:xs)
